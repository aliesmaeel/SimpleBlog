
<article class=" bg-white flex flex-col shadow my-4">
    <!-- Article Image -->
    <a href="{{route('post.view',$post)}}" class="hover:opacity-75">
        <img src="{{App\Helpers\Utilities::getImageFromUrl($post->image)}}">
    </a>
    <div class="bg-white flex flex-col justify-start p-6">
        <div class="flex gap-2">
        @foreach($post->categories as $category)
            <a href="{{route('post-by-category',$category)}}" class="text-blue-700 text-sm font-bold uppercase pb-4">{{$category->title}}</a>
        @endforeach
        </div>
        <a href="{{route('post.view',$post)}}" class="text-3xl font-bold hover:text-gray-700 pb-4">{{$post->title}}</a>
        <p href="{{route('post.view',$post)}}" class="text-sm pb-3">
            By <a href="#" class="font-semibold hover:text-gray-800">{{$post->user->name}}</a>
            , Published on {{App\Helpers\Utilities::getFormattedDate($post->published_at)}}
        </p>
        <a href="{{route('post.view',$post)}}" class="pb-6">{{App\Helpers\Utilities::ShortBody($post->body)}}</a>
        <a href="{{route('post.view',$post)}}" class="uppercase text-gray-800 hover:text-black">Continue Reading <i class="fas fa-arrow-right"></i></a>
    </div>
</article>
